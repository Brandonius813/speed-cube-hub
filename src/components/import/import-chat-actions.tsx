...
export function EventSelectAction({
  ...
  const [seconds, setSeconds] = useState(DEFAULT_SECONDS_PER_SOLVE["333"] ?? 30)
    ...
    <div className="space-y-3 pt-1">
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setSeconds(DEFAULT_SECONDS_PER_SOLVE[e.target.value] ?? 30)
          }}
          disabled={disabled}
      ...
      <div>
        ...
        <input
          type="number"
          value={seconds}
          onChange={(e) => setSeconds(Math.max(1, parseInt(e.target.value) || 30))}
          disabled={disabled}
          className="w-full rounded-lg border border-border/50 bg-background px-3 py-2 text-sm text-foreground"
          min={1}
          max={7200}
      ...
      <Button
        onClick={() => onConfirm(event, seconds)}
        disabled={disabled}
        className="w-full"
        size="sm"
      >
