...
This file translates `.claude/CLAUDE.md`, `.claude/SPEED_CUBE_HUB_PRD.md`, `.claude/TASKS.md`, and `.claude/Rules/*.md` into one Codex-focused operating guide.
...
3. Read `.claude/SPEED_CUBE_HUB_PRD.md`
...
- Push to `dev` only when asked, or in deliberate batches (Vercel preview builds cost minutes)
- Never push broken code
- When user says "go live", merge `dev` into `main` and push

Parallel-session override (when coordinating multiple agents):

- Follow `.claude/TASKS.md` claiming flow (worktree + task branch)
- Claim only one task at a time
- Do not touch files owned by another claimed task
- Log work in `.claude/AGENT_LOG.md`
...
1. Update `.claude/CLAUDE.md` if conventions/routes/key files changed
2. Update `.claude/SPEED_CUBE_HUB_PRD.md` feature status
3. Keep `.claude/TASKS.md` concise (completed tasks collapsed to one-line format)
