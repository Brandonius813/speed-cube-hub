...
- [x] Competition simulation mode: Comp Sim mode entry from the timer practice-type selector, `Mo3`/`Ao5` round formats, cumulative time limits, cutoff after solve 1 or 2, configurable wait-range bounds, longer real crowd-scene ambience, audio-backed inspection calls, selectable local-vs-stationary pre-attempt start flows, optional 60-second sit-down ready window, richer live event shell, explicit post-solve penalty review with retroactive round-sheet edits, Ao5 solve-4 BPA/WPA pressure cues, and cleaned-up Stats-tab Comp Sim tracking with usable filters
