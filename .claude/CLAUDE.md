...
- `src/lib/actions/feed.ts` — Activity feed (getFeed with cursor-based pagination, getFeedEntryById for single-post permalink, enriched with like/comment counts)
- `src/app/(main)/feed/entry/[id]/page.tsx` — Single-post permalink (target of like/comment notifications; tries posts then sessions by UUID)
...
- `src/lib/actions/notifications.ts` — Notifications system (getNotifications with unread-like grouping, markAsRead, markAllAsRead, getUnreadCount)
